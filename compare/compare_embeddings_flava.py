...
class CompareEmbeddingFlava(BaseCompare):
    ...
    def run_comparison(self, store_checkpoints=False):
        ...
        for i in range(self.compare_data.n_files_found):
            ...
            for base_index in similars[0]:
                ...
                if diff_score > CompareEmbeddingFlava.THRESHHOLD_POTENTIAL_DUPLICATE:
                    base_file = self.compare_data.files_found[base_index]
                    diff_file = self.compare_data.files_found[diff_index]
                ...
                if not f1_grouped and not f2_grouped:
                    ...
                    if previous_diff_score - CompareEmbeddingFlava.THRESHHOLD_GROUP_CUTOFF > diff_score:
    ...
    def find_similars_to_image(self, search_path, search_file_index):
        ...
        self.compare_result.finalize_search_result(
            ...
            threshold_duplicate=CompareEmbeddingFlava.THRESHHOLD_POTENTIAL_DUPLICATE,
            threshold_related=CompareEmbeddingFlava.THRESHHOLD_PROBABLE_MATCH)
    ...
    def find_similars_to_embeddings(self, positive_embeddings, negative_embeddings):
        ...
        adjusted_threshold_duplicate = CompareEmbeddingFlava.THRESHHOLD_POTENTIAL_DUPLICATE / normalization_factor
        adjusted_threshold_match = CompareEmbeddingFlava.THRESHHOLD_PROBABLE_MATCH / normalization_factor
    ...
    def _tokenize_text(self, text, embeddings=[], descriptor="search text"):
        if text in CompareEmbeddingFlava.TEXT_EMBEDDING_CACHE:
            text_embedding = CompareEmbeddingFlava.TEXT_EMBEDDING_CACHE[text]
            if text_embedding is not None:
                embeddings.append(CompareEmbeddingFlava.TEXT_EMBEDDING_CACHE[text])
                return
        if self.verbose:
        ...
        try:
            ...
            CompareEmbeddingFlava.TEXT_EMBEDDING_CACHE[text] = text_embedding
        except OSError as e:
            if self.verbose:
                print(f"{text} - {e}")
            raise AssertionError(
    ...
    def single_text_compare(image_path, texts_dict):
        ...
        for key, text in texts_dict.items():
            similarities[key] = embedding_similarity(image_embedding, CompareEmbeddingFlava._get_text_embedding_from_cache(text))
        return similarities

    @staticmethod
    def multi_text_compare(image_path, positives, negatives, threshold=0.3):
        key = (image_path, "::p", tuple(positives), "::n", tuple(negatives))
        if key in CompareEmbeddingFlava.MULTI_EMBEDDING_CACHE:
            return bool(CompareEmbeddingFlava.MULTI_EMBEDDING_CACHE[key] > threshold)
        ...
        for text in positives:
            similarity = embedding_similarity(image_embedding, CompareEmbeddingFlava._get_text_embedding_from_cache(text))
        ...
        CompareEmbeddingFlava.MULTI_EMBEDDING_CACHE[key] = combined_similarity
