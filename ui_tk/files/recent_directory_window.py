...
class RecentDirectoryWindow():
    """Window for selecting recent directories."""
    ...
    def get_history_directory(start_index=0):
        ...
        _dir = None
        for i in range(len(RecentDirectories.directory_history)):
            if i < start_index:
                continue
            _dir = RecentDirectories.directory_history[i]
            break
        return _dir

    @staticmethod
    def update_history(_dir):
        if len(RecentDirectories.directory_history) > 0 and \
                _dir == RecentDirectories.directory_history[0]:
            return
        RecentDirectories.directory_history.insert(0, _dir)
        if len(RecentDirectories.directory_history) > RecentDirectories.MAX_RECENT_DIRECTORIES:
            del RecentDirectories.directory_history[-1]
