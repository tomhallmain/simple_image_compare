...
class ImageOps:
    ...
    def is_video_file(path: str) -> bool:
        """True when *path* is a configured video extension and videos are enabled."""
        if not path or not os.path.isfile(path):
            return False
        if not getattr(config, "enable_videos", True):
            return False
        ext = os.path.splitext(path)[1].lower()
        return ext in {str(e).lower() for e in getattr(config, "video_types", [])}
    ...
    def strip_video_audio(video_path: str) -> str:
        ...
        """
        if not ImageOps.is_video_file(video_path):
            raise RuntimeError("Not a video file")
        ffmpeg = ImageOps.find_ffmpeg_executable()
        if not ffmpeg:
